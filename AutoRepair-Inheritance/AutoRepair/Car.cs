...
namespace AutoRepair
{
    public class Car
    {
        ...
        public List<string> SortedRepairStrings()
        {
            List<string> result = new List<string>();
            foreach (Repair r in repairs)
            {
                string s = string.Format("{0}: {1:C} - {2}", r.RepairID, r.Amount, r.RepairDate.ToShortDateString());
                result.Add(s);
                result.Sort();
            }
            return result;
        ...
        public List<string> SortedRepairByDates()
        {
            List<string> result = new List<string>();
            foreach (Repair r in repairs)
            {
        ...
        public void SortRepairsByDateDelegate()
        {
            Comparison<Repair> sorter = new Comparison<Repair>(RepairDelegates.DateCompare);
            repairs.Sort(sorter);
        }

        public List<Repair> BigRepairs()
        {
            Predicate<Repair> searcher = new Predicate<Repair>(RepairDelegates.BigRepairs);
            List<Repair> results = repairs.FindAll(searcher);
            return results;
        }
